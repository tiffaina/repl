...
test("failed load_file, failed search, successful load_file, a failed search, then a successful search, verbose mode", async ({
  ...
  await page.getByRole("button", { name: "Submitted 1 times" }).click();
  await expect(page.locator(".repl-history")).toContainText("file not found. please try again. hint: you may have the wrong file");
  ...
  // Submit command
  await page.getByRole("button", { name: "Submitted 2 times" }).click();
  await expect(page.locator(".repl-history")).toContainText("search 0 0");
  await expect(page.locator(".repl-history")).toContainText("Backend has not been loaded");
  ...
  await page.getByRole("button", { name: "Submitted 4 times" }).click();
  await expect(page.locator(".repl-history")).toContainText("search 1 nonexistent-value");
  await expect(page.locator(".repl-history")).toContainText( "No results found");
