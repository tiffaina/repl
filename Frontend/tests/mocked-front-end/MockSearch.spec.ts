...
test("call search after failed load_file, without parameters, verbose mode", async ({
  ...
  await page
    .getByLabel("Command input")
    .fill("mock_load_file data/non-existent-file.csv true");
  await page.getByRole("button", { name: "Submitted 1 times" }).click();
  await expect(page.locator(".repl-history")).toContainText("Error: data/non-existent-file.csv not found");
  // Write into command box
  await expect(page.getByLabel("Command input")).toBeVisible();
  await page.getByLabel("Command input").click();
  await page.getByLabel("Command input").fill("mock_search 0 0");
  // Submit command
  await page.getByRole("button", { name: "Submitted 2 times" }).click();

  await expect(page.locator(".repl-history")).toContainText("Command: modeOutput: Mode success!Data:Command: mock_load_file data/non-existent-file.csv trueOutput: Error: data/non-existent-file.csv not foundData:Command: mock_search 0 0Output: Error: CSV file could not be searched. Load correct filepath firstData:");
});
...
test("call search after loaded earnings disparity csv, col and val string has space, verbose mode", async ({
  ...
  await page
    .getByLabel("Command input")
    .fill("mock_load_file data/ten-star.csv true");
  await page.getByRole("button", { name: "Submitted 1 times" }).click();
  ...
  await page
    .getByLabel("Command input")
    .fill('mock_search StarID 0');
  // Submit command
  await page.getByRole("button", { name: "Submitted 2 times" }).click();
  await expect(page.locator(".repl-history")).toContainText("Search success!");
  // Check that table shows up
  await expect(page.locator(".repl-history")).toContainText("0Sol000");
});
